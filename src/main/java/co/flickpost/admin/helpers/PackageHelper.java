...
public class PackageHelper {
    ...
    public static void updateChargeableWeight(Package pkg){
        ...
        if(pkg.getAuditedActualWeight() == null || pkg.getAuditedVolumetricWeight() == null || pkg.getAuditedChargeableWeight() != null){
            return;
        }

        BigDecimal chargeableWeight;
        BigDecimal auditedActualWeight = pkg.getAuditedActualWeight();
        BigDecimal auditedVolumetricWeight = pkg.getAuditedVolumetricWeight();

        chargeableWeight = auditedActualWeight.compareTo(auditedVolumetricWeight) == 1
                ? auditedActualWeight : auditedVolumetricWeight;
    ...
    public static void applyRounding(List<Package> packages){
        for(Package pkg : packages){
            ...
            pkg.setAuditedActualWeight(pkg.getAuditedActualWeight().setScale(2, RoundingMode.UP));
