...
public class SessionPackageEvaluationService {
    ...
    public SessionPackageEvaluationResponse evaluate(SessionPackageEvaluationRequest request) {
        ...
        if (Boolean.TRUE.equals(request.getIsFilled())) {
            applyPayloadValues(sessionPackage, request);
            ...
            return new SessionPackageEvaluationResponse(true, sessionPackage.getTrackingNumber(), finalStatus, REFERENCE_SOURCE_API, false, "SessionPackage evaluated successfully");
        }
        ...
        if (packageReference == null) {
            ...
            sessionPackageDao.singleUpdate(sessionPackage);
            return new SessionPackageEvaluationResponse(true, sessionPackage.getTrackingNumber(), PENDING, sessionPackage.getReferenceSource(), false, "PackageReference not found. SessionPackage set to PENDING");
        }

        applyPackageReferenceValues(sessionPackage, packageReference);
        String finalStatus = determineFinalStatus(
                sessionPackage.getContainsLiquid(),
                sessionPackage.getContainsBattery(),
                sessionPackage.getItemCondition(),
        ...
        return new SessionPackageEvaluationResponse(true, sessionPackage.getTrackingNumber(), finalStatus, REFERENCE_SOURCE_DB, true, "SessionPackage evaluated successfully");
    }
    ...
    private void applyPayloadValues(SessionPackage sessionPackage, SessionPackageEvaluationRequest request) {
        if (request.getShippingMode() != null) {
            sessionPackage.setShippingMode(request.getShippingMode());
        }
        if (request.getContainsLiquid() != null) {
            sessionPackage.setContainsLiquid(request.getContainsLiquid());
        }
        if (request.getContainsBattery() != null) {
            sessionPackage.setContainsBattery(request.getContainsBattery());
        }
        if (request.getItemCondition() != null) {
            sessionPackage.setItemCondition(request.getItemCondition());
        }
        if (request.getIsCommercialPackaging() != null) {
            sessionPackage.setIsCommercialPackaging(request.getIsCommercialPackaging());
        }
        if (request.getDeclaredWeight() != null) {
            sessionPackage.setDeclaredWeight(request.getDeclaredWeight());
        }
        if (request.getAuditedWeight() != null) {
            sessionPackage.setAuditedWeight(request.getAuditedWeight());
        }
        if (request.getAuditedVolumetricWeight() != null) {
            sessionPackage.setAuditedVolumetricWeight(request.getAuditedVolumetricWeight());
        }
        if (request.getDeclaredLength() != null) {
            sessionPackage.setDeclaredLength(request.getDeclaredLength());
        }
        if (request.getDeclaredWidth() != null) {
            sessionPackage.setDeclaredWidth(request.getDeclaredWidth());
        }
        if (request.getDeclaredHeight() != null) {
            sessionPackage.setDeclaredHeight(request.getDeclaredHeight());
        }
        if (request.getClientPaidHeight() != null) {
            sessionPackage.setClientPaidHeight(request.getClientPaidHeight());
        }
    }

    private void applyPackageReferenceValues(SessionPackage sessionPackage, PackageReference packageReference) {
    ...
    private String determineFinalStatus(Boolean containsLiquid,
                                        ...
                                        boolean filledFromPayload,
                                        boolean dataAvailable) {
        if (!filledFromPayload && !dataAvailable) {
            return PENDING;
        }
